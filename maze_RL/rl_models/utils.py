...
def get_plot_and_chkpt_dir(config, participant_name):
    ...
    total_number_updates = config['Experiment'][loop]['total_update_cycles']
    participant = participant_name
    learn_every = config['Experiment'][loop]['learn_every_n_episodes']
...
def get_sac_agent(config, env, chkpt_dir=None):
    ...
    if discrete:
        if config['Experiment']['loop'] == 'max_episodes_mode':
            buffer_max_size = config['Experiment']['max_episodes_mode']['buffer_memory_size']
            update_interval = config['Experiment']['max_episodes_mode']['learn_every_n_episodes']
            scale = config['Experiment']['max_episodes_mode']['reward_scale']

        elif config['Experiment']['loop'] == 'max_interactions_mode':
